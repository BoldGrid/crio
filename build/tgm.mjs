...
//import { setOptions, getData } from './modules/github-request';
...
import( './modules/github-request.js' ).then( ( { setOptions, getData } ) => {
	console.log( chalk.cyanBright( '\nInstalling TGM Plugin Activation Library...\n' ) );
	setOptions( options.calls.latestVersion );
	getData( ( status, response ) => {
		if ( 200 === status ) {
	...
	function getFile( tag, file ) {
		...
		setOptions( Object.assign( options.calls.getFile, url ) );
		getData( ( status, response ) => {
			if ( 200 === status ) {
	...
	function writeFile( content, file ) {
		...
		mkdirp( options.installPath ).then( ( made ) => {
