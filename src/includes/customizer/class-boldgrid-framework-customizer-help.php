...
class Boldgrid_Framework_Customizer_Help {
	...
	public static function get_onboarding_videos() {
		...
		foreach ( $onb_videos as $key => $video ) {
			if ( 'crio' === $video->Plugin ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName
		...
		if ( 0 !== count( $crio_onb_vidos ) ) {
			usort(
				$crio_onb_vidos,
				function( $a, $b ) {
					return $a->DisplayOrder - $b->DisplayOrder; // phpcs:ignore WordPress.NamingConventions.ValidVariableName
				}
			);
		}
	...
	public function get_video_list() {
		...
		if ( 0 !== count( $this->onboarding_videos ) ) {
			...
			foreach ( $this->onboarding_videos as $video ) {
				...
				$video_list .= $video->VideoId; // phpcs:ignore WordPress.NamingConventions.ValidVariableName
				$video_list .= '" class="button button-secondary">';
				$video_list .= $video->Title; // phpcs:ignore WordPress.NamingConventions.ValidVariableName
				$video_list .= '</span>';
				$video_list .= '</li>';
			}
			$video_list .= '</ul>';
